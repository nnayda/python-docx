...
from typing import IO, Dict, Iterator, Optional, Union, cast

from .constants import MIME_TYPE, TIFF_FLD, TIFF_TAG
from .helpers import BIG_ENDIAN, LITTLE_ENDIAN, StreamReader
from .image import BaseImageHeader

# The value parsed from an IFD entry; its concrete type depends on the entry's
# TIFF field type (ASCII -> str, SHORT/LONG -> int, RATIONAL -> float).
_IfdValue = Union[int, float, str]
...
class Tiff(BaseImageHeader):
    ...
    def from_stream(cls, stream: IO[bytes]) -> Tiff:
        ...
        # For a standalone TIFF the ImageWidth/ImageLength tags are always present.
        # When this Tiff is parsed from an Exif APP1 segment they may be absent, but in
        # that case only the dpi values are consumed downstream.
        px_width = cast("int", parser.px_width)
        px_height = cast("int", parser.px_height)
...
class _TiffParser:
    ...
    def __init__(self, ifd_entries: _IfdEntries):
    ...
    @property
    def px_height(self) -> Optional[int]:
        ...
        image."""
        # IMAGE_LENGTH is a SHORT/LONG field, so its value is always an int.
        return cast("Optional[int]", self._ifd_entries.get(TIFF_TAG.IMAGE_LENGTH))

    @property
    def px_width(self) -> Optional[int]:
        ...
        image."""
        # IMAGE_WIDTH is a SHORT/LONG field, so its value is always an int.
        return cast("Optional[int]", self._ifd_entries.get(TIFF_TAG.IMAGE_WIDTH))
    ...
    def _dpi(self, resolution_tag: int) -> int:
        ...
        # the X/Y resolution tags are RATIONAL fields, so the value is numeric.
...
class _IfdEntries:
    ...
    def __init__(self, entries: Dict[int, _IfdValue]):
    ...
    def __getitem__(self, key: int) -> _IfdValue:
    ...
    def get(self, tag_code: int, default: Optional[_IfdValue] = None) -> Optional[_IfdValue]:
...
class _IfdParser:
    ...
    def __init__(self, stream_rdr: StreamReader, offset: int):
...
class _IfdEntry:
    ...
    def __init__(self, tag_code: int, value: _IfdValue):
        super(_IfdEntry, self).__init__()
        self._tag_code = tag_code
        self._value = value

    @classmethod
    def from_stream(cls, stream_rdr: StreamReader, offset: int) -> _IfdEntry:
