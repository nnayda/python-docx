...
class DescribeParagraphFormat:
    def it_knows_its_alignment_value(
        self, alignment_get_fixture: tuple[ParagraphFormat, WD_ALIGN_PARAGRAPH | None]
    ) -> None:
    ...
    def it_can_change_its_alignment_value(
        self, alignment_set_fixture: tuple[ParagraphFormat, WD_ALIGN_PARAGRAPH | None, str]
    ) -> None:
    ...
    def it_knows_its_space_before(
        self, space_before_get_fixture: tuple[ParagraphFormat, Length | None]
    ) -> None:
        paragraph_format, expected_value = space_before_get_fixture
        assert paragraph_format.space_before == expected_value

    def it_can_change_its_space_before(
        self, space_before_set_fixture: tuple[ParagraphFormat, Length | None, str]
    ) -> None:
        paragraph_format, value, expected_xml = space_before_set_fixture
        paragraph_format.space_before = value
        assert paragraph_format._element.xml == expected_xml

    def it_knows_its_space_after(
        self, space_after_get_fixture: tuple[ParagraphFormat, Length | None]
    ) -> None:
        paragraph_format, expected_value = space_after_get_fixture
        assert paragraph_format.space_after == expected_value

    def it_can_change_its_space_after(
        self, space_after_set_fixture: tuple[ParagraphFormat, Length | None, str]
    ) -> None:
        paragraph_format, value, expected_xml = space_after_set_fixture
        paragraph_format.space_after = value
        assert paragraph_format._element.xml == expected_xml

    def it_knows_its_line_spacing(
        self, line_spacing_get_fixture: tuple[ParagraphFormat, float | Length | None]
    ) -> None:
    ...
    def it_can_change_its_line_spacing(
        self, line_spacing_set_fixture: tuple[ParagraphFormat, float | Length | None, str]
    ) -> None:
    ...
    def it_knows_its_line_spacing_rule(
        self, line_spacing_rule_get_fixture: tuple[ParagraphFormat, WD_LINE_SPACING | None]
    ) -> None:
        paragraph_format, expected_value = line_spacing_rule_get_fixture
        assert paragraph_format.line_spacing_rule == expected_value

    def it_can_change_its_line_spacing_rule(
        self, line_spacing_rule_set_fixture: tuple[ParagraphFormat, WD_LINE_SPACING | None, str]
    ) -> None:
        paragraph_format, value, expected_xml = line_spacing_rule_set_fixture
        paragraph_format.line_spacing_rule = value
        assert paragraph_format._element.xml == expected_xml

    def it_knows_its_first_line_indent(
        self, first_indent_get_fixture: tuple[ParagraphFormat, Length | None]
    ) -> None:
        paragraph_format, expected_value = first_indent_get_fixture
        assert paragraph_format.first_line_indent == expected_value

    def it_can_change_its_first_line_indent(
        self, first_indent_set_fixture: tuple[ParagraphFormat, Length | int | None, str]
    ) -> None:
        paragraph_format, value, expected_xml = first_indent_set_fixture
        paragraph_format.first_line_indent = cast("Length | None", value)
        assert paragraph_format._element.xml == expected_xml

    def it_knows_its_left_indent(
        self, left_indent_get_fixture: tuple[ParagraphFormat, Length | None]
    ) -> None:
    ...
    def it_can_change_its_left_indent(
        self, left_indent_set_fixture: tuple[ParagraphFormat, Length | int | None, str]
    ) -> None:
        paragraph_format, value, expected_xml = left_indent_set_fixture
        paragraph_format.left_indent = cast("Length | None", value)
        assert paragraph_format._element.xml == expected_xml

    def it_knows_its_right_indent(
        self, right_indent_get_fixture: tuple[ParagraphFormat, Length | None]
    ) -> None:
        paragraph_format, expected_value = right_indent_get_fixture
        assert paragraph_format.right_indent == expected_value

    def it_can_change_its_right_indent(
        self, right_indent_set_fixture: tuple[ParagraphFormat, Length | int | None, str]
    ) -> None:
        paragraph_format, value, expected_xml = right_indent_set_fixture
        paragraph_format.right_indent = cast("Length | None", value)
        assert paragraph_format._element.xml == expected_xml

    def it_knows_its_on_off_prop_values(
        self, on_off_get_fixture: tuple[ParagraphFormat, str, bool | None]
    ) -> None:
        paragraph_format, prop_name, expected_value = on_off_get_fixture
        assert getattr(paragraph_format, prop_name) == expected_value

    def it_can_change_its_on_off_props(
        self, on_off_set_fixture: tuple[ParagraphFormat, str, bool | None, str]
    ) -> None:
        paragraph_format, prop_name, value, expected_xml = on_off_set_fixture
        setattr(paragraph_format, prop_name, value)
        assert paragraph_format._element.xml == expected_xml

    def it_provides_access_to_its_tab_stops(
        self, tab_stops_fixture: tuple[ParagraphFormat, Mock, CT_PPr, Mock]
    ) -> None:
    ...
    ) -> tuple[ParagraphFormat, WD_ALIGN_PARAGRAPH | None]:
        p_cxml, expected_value = cast("tuple[str, WD_ALIGN_PARAGRAPH | None]", request.param)
    ...
    def alignment_set_fixture(
        self, request: FixtureRequest
    ) -> tuple[ParagraphFormat, WD_ALIGN_PARAGRAPH | None, str]:
        p_cxml, value, expected_cxml = cast(
    ...
    def first_indent_get_fixture(
        self, request: FixtureRequest
    ) -> tuple[ParagraphFormat, Length | None]:
        p_cxml, expected_value = cast("tuple[str, Length | None]", request.param)
        paragraph_format = ParagraphFormat(cast(CT_P, element(p_cxml)))
        return paragraph_format, expected_value
    ...
    def first_indent_set_fixture(
        self, request: FixtureRequest
    ) -> tuple[ParagraphFormat, Length | int | None, str]:
        p_cxml, value, expected_p_cxml = cast("tuple[str, Length | int | None, str]", request.param)
        paragraph_format = ParagraphFormat(cast(CT_P, element(p_cxml)))
    ...
    def line_spacing_get_fixture(
        self, request: FixtureRequest
    ) -> tuple[ParagraphFormat, float | Length | None]:
        p_cxml, expected_value = cast("tuple[str, float | Length | None]", request.param)
        paragraph_format = ParagraphFormat(cast(CT_P, element(p_cxml)))
        return paragraph_format, expected_value
    ...
    def line_spacing_set_fixture(
        self, request: FixtureRequest
    ) -> tuple[ParagraphFormat, float | Length | None, str]:
        p_cxml, value, expected_p_cxml = cast(
            "tuple[str, float | Length | None, str]", request.param
        )
        paragraph_format = ParagraphFormat(cast(CT_P, element(p_cxml)))
        expected_xml = xml(expected_p_cxml)
        return paragraph_format, value, expected_xml
    ...
    def line_spacing_rule_get_fixture(
        self, request: FixtureRequest
    ) -> tuple[ParagraphFormat, WD_LINE_SPACING | None]:
        p_cxml, expected_value = cast("tuple[str, WD_LINE_SPACING | None]", request.param)
        paragraph_format = ParagraphFormat(cast(CT_P, element(p_cxml)))
        return paragraph_format, expected_value
    ...
    def line_spacing_rule_set_fixture(
        self, request: FixtureRequest
    ) -> tuple[ParagraphFormat, WD_LINE_SPACING | None, str]:
        p_cxml, value, expected_p_cxml = cast(
            "tuple[str, WD_LINE_SPACING | None, str]", request.param
        )
        paragraph_format = ParagraphFormat(cast(CT_P, element(p_cxml)))
        expected_xml = xml(expected_p_cxml)
        return paragraph_format, value, expected_xml
    ...
    def on_off_get_fixture(
        self, request: FixtureRequest
    ) -> tuple[ParagraphFormat, str, bool | None]:
        p_cxml, prop_name, expected_value = cast("tuple[str, str, bool | None]", request.param)
        paragraph_format = ParagraphFormat(cast(CT_P, element(p_cxml)))
    ...
    def on_off_set_fixture(
        self, request: FixtureRequest
    ) -> tuple[ParagraphFormat, str, bool | None, str]:
        p_cxml, prop_name, value, expected_cxml = cast(
    ...
    def space_after_set_fixture(
        self, request: FixtureRequest
    ) -> tuple[ParagraphFormat, Length | None, str]:
        p_cxml, value, expected_p_cxml = cast("tuple[str, Length | None, str]", request.param)
        paragraph_format = ParagraphFormat(cast(CT_P, element(p_cxml)))
        expected_xml = xml(expected_p_cxml)
        return paragraph_format, value, expected_xml
    ...
    @pytest.fixture
    def tab_stops_fixture(
        self, TabStops_: Mock, tab_stops_: Mock
    ) -> tuple[ParagraphFormat, Mock, CT_PPr, Mock]:
        p = cast(CT_P, element("w:p/w:pPr"))
        pPr = p.pPr
        assert pPr is not None
        paragraph_format = ParagraphFormat(p, cast("t.ProvidesXmlPart", None))
        return paragraph_format, TabStops_, pPr, tab_stops_

    # fixture components ---------------------------------------------

    @pytest.fixture
    def TabStops_(self, request: FixtureRequest, tab_stops_: Mock) -> Mock:
        return class_mock(request, "docx.text.parfmt.TabStops", return_value=tab_stops_)

    @pytest.fixture
    def tab_stops_(self, request: FixtureRequest) -> Mock:
