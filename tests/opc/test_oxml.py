...
class DescribeCT_Relationship:
    ...
    def it_can_construct_from_attribute_values(self) -> None:
        ...
        for rId, reltype, target, target_mode in cases:
            ...
            assert cast(CT_Relationship, rel).xml == expected_rel_xml


class DescribeCT_Relationships:
    def it_can_construct_a_new_relationships_element(self) -> None:
    ...
    def it_can_build_rels_element_incrementally(self) -> None:
        # setup ------------------------
        rels = CT_Relationships.new()
        # exercise ---------------------
    ...
    def it_can_generate_rels_file_xml(self) -> None:
        expected_xml = (
...
class DescribeCT_Types:
    def it_provides_access_to_default_child_elements(self) -> None:
        types = cast(CT_Types, a_Types().element)
        assert len(types.defaults) == 2
        for default in types.defaults:
            assert isinstance(default, CT_Default)

    def it_provides_access_to_override_child_elements(self) -> None:
        types = cast(CT_Types, a_Types().element)
        assert len(types.overrides) == 3
        for override in types.overrides:
            assert isinstance(override, CT_Override)

    def it_should_have_empty_list_on_no_matching_elements(self) -> None:
        types = cast(CT_Types, a_Types().empty().element)
        assert types.defaults == []
        assert types.overrides == []

    def it_can_construct_a_new_types_element(self) -> None:
        types = CT_Types.new()
        expected_xml = a_Types().empty().xml
        assert types.xml == expected_xml

    def it_can_build_types_element_incrementally(self) -> None:
        types = CT_Types.new()
