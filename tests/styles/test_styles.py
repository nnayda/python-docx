...
class DescribeStyles:
    def it_supports_the_in_operator_on_style_name(
        self, in_fixture: tuple[Styles, str, bool]
    ) -> None:
        styles, name, expected_value = in_fixture
        assert (name in styles) is expected_value

    def it_knows_its_length(self, len_fixture: tuple[Styles, int]) -> None:
        styles, expected_value = len_fixture
        assert len(styles) == expected_value

    def it_can_iterate_over_its_styles(
        self, iter_fixture: tuple[Styles, int, Mock, Mock, Sequence[object]]
    ) -> None:
    ...
    def it_can_get_a_style_by_id(self, getitem_id_fixture: tuple[Styles, str, CT_Style]) -> None:
        styles, key, expected_element = getitem_id_fixture
        style = styles[key]
        assert style._element is expected_element

    def it_can_get_a_style_by_name(
        self, getitem_name_fixture: tuple[Styles, str, CT_Style]
    ) -> None:
        styles, key, expected_element = getitem_name_fixture
        style = styles[key]
        assert style._element is expected_element

    def it_raises_on_style_not_found(self, get_raises_fixture: tuple[Styles, str]) -> None:
        styles, key = get_raises_fixture
        with pytest.raises(KeyError):
            styles[key]

    def it_can_add_a_new_style(
        self, add_fixture: tuple[Styles, str, WD_STYLE_TYPE, bool, str, Mock, Mock, Mock]
    ) -> None:
        ...
        cast(Mock, styles._element.add_style_of_type).assert_called_once_with(
            name_, style_type, builtin
        )
        StyleFactory_.assert_called_once_with(style_elm_)
        assert style is style_

    def it_raises_when_style_name_already_used(
        self, add_raises_fixture: tuple[Styles, str]
    ) -> None:
        styles, name = add_raises_fixture
        with pytest.raises(ValueError, match="document already contains style 'Hea"):
            styles.add_style(name, None)  # pyright: ignore[reportArgumentType]

    def it_can_get_the_default_style_for_a_type(
        self, default_fixture: tuple[Styles, WD_STYLE_TYPE, Mock, Sequence[object], object]
    ) -> None:
    ...
    def it_can_get_a_style_of_type_by_id(self, _get_by_id_: Mock, style_: Mock) -> None:
        style_id, style_type = 42, 7
        _get_by_id_.return_value = style_
        styles = Styles(cast(CT_Styles, None))
    ...
    def but_it_returns_the_default_style_for_style_id_None(
        self, default_: Mock, style_: Mock
    ) -> None:
        style_type = 17
    ...
    def it_can_get_a_style_id_from_a_style(self, _get_style_id_from_style_: Mock) -> None:
        style = BaseStyle(cast(CT_Style, None))
        style_type = 22
    ...
    def and_it_can_get_a_style_id_from_a_style_name(self, _get_style_id_from_name_: Mock) -> None:
        style_type = 22
        _get_style_id_from_name_.return_value = "StyleId"
        styles = Styles(cast(CT_Styles, None))

        style_id = styles.get_style_id(
            "Style Name",
            style_type,  # pyright: ignore[reportArgumentType]
        )

        _get_style_id_from_name_.assert_called_once_with(styles, "Style Name", style_type)
        assert style_id == "StyleId"

    def but_it_returns_None_for_a_style_or_name_of_None(self) -> None:
        styles = Styles(cast(CT_Styles, None))
    ...
    def it_gets_a_style_by_id_to_help(
        self,
        _get_by_id_fixture: tuple[
            Styles, str, WD_STYLE_TYPE, Sequence[object], Mock, Sequence[object], Mock
        ],
    ) -> None:
        ...
        assert cast(Mock, styles.default).call_args_list == default_calls
        assert StyleFactory_.call_args_list == StyleFactory_calls
        assert style is style_

    def it_gets_a_style_id_from_a_name_to_help(
        self, _getitem_: Mock, _get_style_id_from_style_: Mock, style_: Mock
    ) -> None:
        ...
        style_id = styles._get_style_id_from_name(
            style_name,
            style_type,  # pyright: ignore[reportArgumentType]
        )

        cast(Mock, styles.__getitem__).assert_called_once_with(styles, style_name)
        _get_style_id_from_style_.assert_called_once_with(styles, style_, style_type)
        assert style_id is style_id_

    def it_gets_a_style_id_from_a_style_to_help(
        self, id_style_fixture: tuple[Styles, Mock, WD_STYLE_TYPE, str | None]
    ) -> None:
        styles, style_, style_type, style_id_ = id_style_fixture

        style_id = styles._get_style_id_from_style(style_, style_type)

        cast(Mock, styles.default).assert_called_once_with(styles, style_type)
        assert style_id is style_id_

    def it_raises_on_style_type_mismatch(
        self, id_style_raises_fixture: tuple[Styles, Mock, WD_STYLE_TYPE]
    ) -> None:
    ...
    def it_provides_access_to_the_latent_styles(
        self, latent_styles_fixture: tuple[Styles, Mock, Mock]
    ) -> None:
    ...
    def add_fixture(
        ...
        StyleFactory_: Mock,
        style_: Mock,
    ) -> tuple[Styles, str, WD_STYLE_TYPE, bool, str, Mock, Mock, Mock]:
        name, name_, style_type, builtin = cast(
            "tuple[str, str, WD_STYLE_TYPE, bool]", request.param
        )
    ...
    @pytest.fixture
    def add_raises_fixture(self, _getitem_: Mock) -> tuple[Styles, str]:
    ...
    def default_fixture(
        self, request: FixtureRequest, StyleFactory_: Mock, style_: Mock
    ) -> tuple[Styles, WD_STYLE_TYPE, Mock, Sequence[object], object]:
        styles_cxml, is_defined, style_type = cast("tuple[str, bool, WD_STYLE_TYPE]", request.param)
        styles_elm = cast(CT_Styles, element(styles_cxml))
    ...
    def _get_by_id_fixture(
        self, request: FixtureRequest, default_: Mock, StyleFactory_: Mock, style_: Mock
    ) -> tuple[Styles, str, WD_STYLE_TYPE, Sequence[object], Mock, Sequence[object], Mock]:
        styles_cxml, style_id, style_type = cast("tuple[str, str, WD_STYLE_TYPE]", request.param)
        styles_elm = cast(CT_Styles, element(styles_cxml))
    ...
    def getitem_id_fixture(self, request: FixtureRequest) -> tuple[Styles, str, CT_Style]:
        styles_cxml_tmpl, style_idx = cast("tuple[str, int]", request.param)
        styles_cxml = styles_cxml_tmpl % "w:type=paragraph"
        styles = Styles(cast(CT_Styles, element(styles_cxml)))
        expected_element = cast(CT_Style, styles._element[style_idx])
    ...
    def getitem_name_fixture(self, request: FixtureRequest) -> tuple[Styles, str, CT_Style]:
        styles_cxml_tmpl, key, style_idx = cast("tuple[str, str, int]", request.param)
    ...
    def get_raises_fixture(self, request: FixtureRequest) -> tuple[Styles, str]:
        styles_cxml = cast(str, request.param)
        styles = Styles(cast(CT_Styles, element(styles_cxml)))
        return styles, "bar"

    @pytest.fixture(params=[True, False])
    def id_style_fixture(
        self, request: FixtureRequest, default_: Mock, style_: Mock
    ) -> tuple[Styles, Mock, WD_STYLE_TYPE, str | None]:
        style_is_default = cast(bool, request.param)
        styles = Styles(cast(CT_Styles, None))
        ...
        return styles, style_, cast(WD_STYLE_TYPE, style_type), expected_value

    @pytest.fixture
    def id_style_raises_fixture(self, style_: Mock) -> tuple[Styles, Mock, WD_STYLE_TYPE]:
        styles = Styles(cast(CT_Styles, None))
        style_.type = 1
        style_type = 2
        return styles, style_, cast(WD_STYLE_TYPE, style_type)
    ...
    def in_fixture(self, request: FixtureRequest) -> tuple[Styles, str, bool]:
        styles_cxml, name, expected_value = cast("tuple[str, str, bool]", request.param)
        styles = Styles(cast(CT_Styles, element(styles_cxml)))
    ...
    def iter_fixture(
        self, request: FixtureRequest, StyleFactory_: Mock, style_: Mock
    ) -> tuple[Styles, int, Mock, Mock, Sequence[object]]:
        styles_cxml, expected_count = cast("tuple[str, int]", request.param)
        styles_elm = cast(CT_Styles, element(styles_cxml))
        styles = Styles(styles_elm)
    ...
    @pytest.fixture
    def latent_styles_fixture(
        self, LatentStyles_: Mock, latent_styles_: Mock
    ) -> tuple[Styles, Mock, Mock]:
    ...
    def len_fixture(self, request: FixtureRequest) -> tuple[Styles, int]:
        styles_cxml, expected_value = cast("tuple[str, int]", request.param)
        styles = Styles(cast(CT_Styles, element(styles_cxml)))
    ...
    @pytest.fixture
    def default_(self, request: FixtureRequest) -> Mock:
        return method_mock(request, Styles, "default")

    @pytest.fixture
    def _get_by_id_(self, request: FixtureRequest) -> Mock:
        return method_mock(request, Styles, "_get_by_id")

    @pytest.fixture
    def _getitem_(self, request: FixtureRequest) -> Mock:
        return method_mock(request, Styles, "__getitem__")

    @pytest.fixture
    def _get_style_id_from_name_(self, request: FixtureRequest) -> Mock:
        return method_mock(request, Styles, "_get_style_id_from_name")

    @pytest.fixture
    def _get_style_id_from_style_(self, request: FixtureRequest) -> Mock:
        return method_mock(request, Styles, "_get_style_id_from_style")

    @pytest.fixture
    def LatentStyles_(self, request: FixtureRequest, latent_styles_: Mock) -> Mock:
        return class_mock(request, "docx.styles.styles.LatentStyles", return_value=latent_styles_)

    @pytest.fixture
    def latent_styles_(self, request: FixtureRequest) -> Mock:
    ...
    @pytest.fixture
    def style_elm_(self, request: FixtureRequest) -> Mock:
